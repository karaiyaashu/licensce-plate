...
def util(path, model):
    ...
    for r in matched_result:
        for d in r:
            # cv2.drawContours(temp_result, d['contour'], -1, (255, 255, 255))
            cv2.rectangle(img_ori, pt1=(d['x'], d['y']), pt2=(d['x'] + d['w'], d['y'] + d['h']), color=(0, 0, 255),
                          thickness=2)
    ...
    if len(plate_imgs) > 0:
        img_result = thresholding_again(plate_imgs)
        img = 255 - img_result
        char = segment_characters(img)

        print("Deep learing: ",show_results(char, model))
...
def thresholding_again(plate_imgs):
